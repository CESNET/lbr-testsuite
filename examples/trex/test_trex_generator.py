...
Simple TRex module example. It generates SYN flood traffic.

For your environment, set correct TARGET_VLAN and TARGET_MAC variables.

Example usage:
    pytest --trex-generator="trex,0000:65:00.0" -s --log-level=info
"""
...
def init_trex_manager(trex_generators):
    trex_manager = trex.TRexManager(trex.TRexMachinesPool(trex_generators))
    return trex_manager


def test_trex_syn_flood(request: pytest.FixtureRequest, trex_generators: dict):
    trex_manager = init_trex_manager(trex_generators)
